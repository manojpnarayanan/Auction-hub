...
export class GetUserBidUseCase implements IGetUserBidsUseCase {
    ...
    async execute(userId: string, page: number, limit: number): Promise<{ data: UserBidResponseDTO[], total: number }> {
        ...
        const data: UserBidResponseDTO[] = auctions.filter(auction => auction !== null)
            .map(auction => {
                const myBidsforAuction = bids.filter(bid => bid?.auctionId?.toString() === auction.id!.toString());
                if (myBidsforAuction.length === 0) return null;
                const myHighestBid = myBidsforAuction[0].amount;
                const isHighestBidder = auction!.currentPrice === myHighestBid;

                const lastBidTime = myBidsforAuction.reduce((latest, bid) =>
                    bid.time.getTime() > latest.getTime() ? bid.time : latest,
                    myBidsforAuction[0].time
                );
            ...
            }).filter(item => item !== null) as UserBidResponseDTO[];
        return {
            data: data.sort((a, b) => {
                const timeA = a.lastBidTime instanceof Date ? a.lastBidTime.getTime() : 0;
                const timeB = b.lastBidTime instanceof Date ? b.lastBidTime.getTime() : 0;
                return timeB - timeA;
            }),
            total
        };
