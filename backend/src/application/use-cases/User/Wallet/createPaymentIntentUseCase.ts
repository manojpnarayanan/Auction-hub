...
export class CreatePaymentIntentUseCase implements ICreatePaymentIntentUseCase{
    ...
    async execute(buyerId: string, data: createPaymentIntentDTO): Promise<PaymentIntentResponse> {
        let wallet=await this.walletRepository.findByUserId(buyerId);
        if(!wallet) {
            wallet=await this.walletRepository.create(buyerId)
        }
        const intent=await this.paymentService.createPaymentIntent(data.amount,'inr',{auctionId:data.auctionId,buyerId});
