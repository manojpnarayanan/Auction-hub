...
export class SignupUseCase implements ISignupUseCase {
    ...
    async execute(userData: CreateUserDTO): Promise<LoginResponseDTO> {
        ...
        if (existingUser) {
            ...
            user = await this._userRepository.updateUnVerifiedUser(existingUser?.id, { ...userData, password: hashedPassword });

        } else {
            const hashedPassword = await bcrypt.hash(userData.password, 10);
            user = await this._userRepository.create({ ...userData, password: hashedPassword })
        }
